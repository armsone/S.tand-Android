...
상태: Android·iPhone 동시 구현 기준. 인터넷 중계는 후순위다.

보이소는 한 돌봄 공간에 `볼 사람`과 `말할 사람` 기기를 각각 여러 대 연결한다. 가까운
...
원음과 녹음 파일은 어떤 전송 경로로도 보내지 않는다. 사용자에게는 `볼 사람·말할 사람`만
표시하고 내부 wire value는 기존 호환을 위해 각각 `host·guest`를 유지한다.

## QR 초대

첫 볼 사람 한 명만 새 공간을 만들고 초대 QR을 표시한다. 참여 기기는 QR을 촬영한 뒤 역할과
...
보이소 소리 사건과 각 플랫폼의 수면 녹음·화들짝은 같은 입력 기준을 사용한다. 플랫폼별
마이크 특성은 허용하되 아래 경계값과 무반응 보장은 동일해야 한다.

- 매이트 감시를 시작한 첫 60초는 주변 소리 학습 구간이다. 레벨 표시는 가능하지만 녹음 후보,
  코골이·잠꼬대 승인, `big_sound`·`continuous_sound`·`finger_snap`, 로컬 화들짝을 만들지 않는다.
- 학습 뒤 RMS 문턱은 `max(사용자 감도, 주변 바닥 소음 + 여유폭)`이다. 여유폭은 바닥 소음이
  `-50 dB` 미만이면 10dB, `-50..< -35 dB`이면 12dB, 그 이상이면 14dB이며 최종 범위는
  `-58...-18 dB`다. 자동 학습은 사용자가 고른 감도보다 민감하게 만들 수 없다.
- 일반 소리는 RMS 문턱을 60ms 이상 통과해야 시작한다. `continuous_sound`는 같은 보수적
  문턱을 2초 이상 유지해야 한다.
- 박수·핑거스냅 피크 문턱은 일반 소리와 분리한다. 기본 피크 `-18 dB`보다 낮은 소리는
  순간 상승폭만으로 `finger_snap`이나 화들짝을 만들지 않는다.
- 코골이·잠꼬대만 녹음을 승인한다. 뒤척임은 화들짝 후보지만 저장하지 않고, 분류가 애매한
  생활 소음·외부 TV·라디오는 저장하거나 화들짝을 만들지 않는다. 앱 자체 라디오 재생 중에는
  기존처럼 마이크 감지와 녹음을 중지한다.
- 기준 회귀 벡터의 AAC 재디코딩 수치는 전체 RMS `-53.19 dB`, 최대 20ms RMS `-46.69 dB`,
  피크 `-36.92 dB`다. 권장 사용자 감도 `-36 dB`, 기본 박수 피크 `-18 dB`에서
  녹음·화들짝·보이소 소리 사건이 모두 0이어야 한다.

원음과 녹음은 계속 기기 밖으로 보내지 않는다. 보이소는 위 기준을 통과한 사건 종류와
정규화된 강도만 전송한다.
