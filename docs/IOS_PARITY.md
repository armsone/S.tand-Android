...
Android 기준: 작업 트리의 versionName `0.0.1`, versionCode `25`, 빌드 `0.0.25`
...
|---|---|---|---|---|---|
| 전체 화면·진입점 | 구현 | 해당 없음 | 최신 소스 대기 | 최신 소스 대기 | 별도 채널 관리, 홈 라디오 길게 편집, 브라우저 3경로와 편집 중 빈 라디오의 관리 진입 구현 |
...
| 앱 내부 브라우저 | **부분 구현** | 주소·검색·credential·즐겨찾기 테스트 통과 | 대기 | 이전 versionCode 25 두 기기 설치, 최신 소스 브라우저 UI 대기 | 설정/관리/편집 3경로, renderer 재생성, 파일 선택·drop·WebView 문맥 메뉴 차단 구현. popup·미디어 실검증은 남음 |
...
- Android versionName `0.0.1`, versionCode `25`, 빌드 표기 `0.0.25`.
- 로컬 JVM 테스트·`assembleDebug`·`lintDebug` 성공.
- `SM-T500`에는 이전 0.0.25 APK가 설치되어 Activity 실행·홈 footer 표시까지 확인됨.
- `SM-F968N`에는 이전 0.0.25 APK가 설치되어 versionCode 25까지 확인됨.
- 이 문서의 최신 소스 변경은 코드 구현까지만 진행했으며 두 기기에 설치하지 않았다.
- 커밋·푸시는 아직 수행하지 않았다.
