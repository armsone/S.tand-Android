...
2. 첫 볼사람은 `새 보이소 만들기`로 v2 QR을 생성한다.
3. 다른 볼사람·말할사람은 카메라로 QR을 촬영하고 역할·기기 이름을 정한다.
...
- 볼사람 2대·말할사람 2대가 한 공간에 동시에 나타난다.
...
- versionCode 44 전체 JVM 테스트·`lintDebug`·`assembleDebug` 통과
- `SM-T500`에서 매이트 잠금 자물쇠 표시, 화들짝 동안 자물쇠 숨김, 우는 아이 알림 화면
  계측 테스트 3개 통과
- `SM-F968N`과 `SM-T500`에 같은 versionCode 44 APK를 데이터 유지 설치하고 실행 확인
