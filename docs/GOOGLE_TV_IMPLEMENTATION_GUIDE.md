...
- TV 리모컨으로 긴 방송 주소를 입력하지 않아도 되도록 라디오 관리 화면에서 `휴대전화에서 받기`를 제공한다.
- TV가 표시하는 일회용 QR 주소는 같은 Wi-Fi의 휴대전화 브라우저에서만 짧게 사용하며, 외부 서버나 계정을 거치지 않는다. 전송 성공, 취소 또는 3분 만료 시 로컬 수신기를 닫는다.
- 받은 파일은 `format: s.tand-radio`, `version: 1`인 128KB 이하 JSON만 허용하고 최대 4개 채널을 미리 보여준 뒤 사용자가 확인해야 저장한다. 자동 재생이나 기존 채널 자동 덮어쓰기는 금지한다.
- QR 전송이 막힌 환경에서는 USB 또는 시스템 문서 선택기의 `파일에서 가져오기`를 안내한다.
- 사용자 소유 설정에는 `http://`와 `https://` 스트림을 모두 허용한다. HTTP는 `암호화되지 않은 방송 주소`로 표시하고, 사용자 정보가 포함된 URL은 계속 거부한다.
