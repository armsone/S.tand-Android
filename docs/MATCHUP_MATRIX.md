...
| `boyiso_walkie_setup` | role choice | title/description/wire value | disconnected/ko-KR | `무전기`, exact description, `walkie` | same source values | runtime geometry not yet measured | paired phone/tablet capture | source High | 구현, 캡처 확인 필요 |
| `boyiso_walkie_connected` | primary call control | label/hint/action/cooldown | connected walkie | `무전기 호출`, exact hint, 3s cooldown | same label/hint/action/cooldown | icon geometry and rendered placement pending | paired capture + repeated-tap trace | source High | 구현, 캡처 확인 필요 |
| `boyiso_walkie_received` | full-screen reaction | sender/message/light/chime/duration | active mate session | warm high-salience overlay, urgent light, 2 chimes, 3s | same semantic reaction | rendered asset/bounds and timing trace pending | paired capture + timestamped trace | source High | 구현, 실기기 확인 필요 |
| `boyiso_walkie_background` | system notification | title/body/tap destination | app background | iOS notification, `무전기 호출이 왔어요.` | Android high-priority notification and app reopen | OS-owned notification chrome differs | verify payload/tap; mask OS chrome only | source High | forced OS exception 후보, 실기기 확인 필요 |
